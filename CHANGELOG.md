...
- Added a compact expandable Network composer: the message editor and posting options remain concealed on every page load and open only when the transmission field is selected; the editor then grows with typed content before switching to an internal scrollbar at a monitor-aware maximum height.
