...
- Pilot Social feed, posting, comments, editing, deletion, or reactions;
- Pilot Social notification badges, filters, read/unread state, or category preferences;
- Private Pilot Social transmissions;
- Birthday privacy or reminders;
- Paper Airplane eligibility, cooldown, outcome display, or leaderboard synchronization;
- Community feed;
- Groups;
- Events;
- Chat;
...
For a Pilot Social report, include whether the problem occurred on the website, in USAFCACARS, or both; the selected notification filter; the approximate timestamp; and the exact action taken. Do not include private message contents, credentials, private access codes, or another pilot's non-public profile information in a public issue.

Paper Airplane scores are generated by the server. A client-side result, altered request, or screenshot is not authoritative; include the interaction time and involved callsigns privately so support can locate the stored outcome.
