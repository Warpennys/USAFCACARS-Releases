...
- The complete inventory remains searchable while a finite, vertically scrollable viewport renders paged, recycled rows. Each visible page and opened paintjob selector hydrates only from the app-local cache on bounded background image workers, deduplicates simultaneous requests, and persists optimized thumbnails for immediate reuse across sessions; browsing never waits on the API or simulator.
